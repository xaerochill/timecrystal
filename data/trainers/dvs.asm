TrainerClassDVs:
; entries correspond to trainer classes (see constants/trainer_constants.asm)
	table_width 2, TrainerClassDVs
	;  atk,def,spd,spc
	dn 15, 13, 15, 15 ; FALKNER (hp ice)
	dn  7, 15, 15, 15 ; WHITNEY (female for 50/50 gender ratio and below)
	dn 13, 13, 15, 15 ; BUGSY (hp bug)
	dn 13, 13, 15, 15 ; MORTY (hp bug)
	dn 14, 15, 15, 15 ; PRYCE (hp electric)
	dn 14, 13, 15, 15 ; JASMINE (hp water)
	dn 13, 14, 15, 15 ; CHUCK (hp ghost)
	dn 12, 15, 15, 15 ; CLAIR (hp ground)
	dn 13, 13, 13, 13 ; RIVAL1
	dn 15, 15, 15, 15 ; POKEMON_PROF
	dn 14, 12, 15, 15 ; WILL (hp fire)
	dn 15, 15, 15, 15 ; CAL
	dn 13, 14, 15, 15 ; BRUNO (hp ghost)
	dn 15, 15, 15, 15 ; KAREN
	dn 12, 15, 15, 15 ; KOGA (hp ground)
	dn 13, 12, 15, 15 ; CHAMPION (hp rock)
	dn 14, 15, 15, 15 ; BROCK (hp electric)
	dn 14, 14, 15, 15 ; MISTY (hp grass)
	dn 15, 13, 15, 15 ; LT_SURGE (hp ice)
	dn  9,  8,  8,  8 ; SCIENTIST
	dn 15, 13, 15, 15 ; ERIKA (hp ice)
	dn  9,  8,  8,  8 ; YOUNGSTER
	dn  9,  8,  8,  8 ; SCHOOLBOY
	dn  9,  8,  8,  8 ; BIRD_KEEPER
	dn  5,  8,  8,  8 ; LASS
	dn  7, 15, 15, 15 ; JANINE (female for 50/50 gender ratio and below)
	dn 13,  8, 12,  8 ; COOLTRAINERM
	dn  7, 12, 12,  8 ; COOLTRAINERF
	dn  6,  9, 12,  8 ; BEAUTY
	dn  9,  8,  8,  8 ; POKEMANIAC
	dn 13,  8, 10,  8 ; GRUNTM
	dn  9,  8,  8,  8 ; GENTLEMAN
	dn  9,  8,  8,  8 ; SKIER
	dn  6,  8,  8,  8 ; TEACHER_F
	dn 14, 12, 15, 15 ; SABRINA (hp fire)
	dn  9,  8,  8,  8 ; BUG_CATCHER
	dn  9,  8,  8,  8 ; FISHER
	dn  9,  8,  8,  8 ; SWIMMERM
	dn  7,  8,  8,  8 ; SWIMMERF
	dn  9,  8,  8,  8 ; SAILOR
	dn  9,  8,  8,  8 ; SUPER_NERD
	dn 13, 13, 13, 13 ; RIVAL2
	dn  9,  8,  8,  8 ; GUITARIST
	dn 10,  8,  8,  8 ; HIKER
	dn  9,  8,  8,  8 ; BIKER
	dn 14, 14, 15, 15 ; BLAINE (hp grass)
	dn  9,  8,  8,  8 ; BURGLAR
	dn  9,  8,  8,  8 ; FIREBREATHER
	dn  9,  8,  8,  8 ; JUGGLER
	dn  9,  8,  8,  8 ; BLACKBELT_T
	dn 13, 13, 15, 15 ; ARCHER (hp bug)
	dn  9,  8,  8,  8 ; PSYCHIC_T
	dn  6, 10, 10,  8 ; PICNICKER
	dn  9,  8,  8,  8 ; CAMPER
	dn  7, 15, 15, 15 ; ARIANA (female for 50/50 gender ratio and below)
	dn  9,  8,  8,  8 ; SAGE
	dn  7,  8,  8,  8 ; MEDIUM
	dn  9,  8,  8,  8 ; BOARDER
	dn  9,  8,  8,  8 ; POKEFANM
	dn  6,  8,  8, 10 ; KIMONO_GIRL
	dn  6,  8, 10,  8 ; TWINS
	dn  6, 13,  8,  8 ; POKEFANF
	dn 15, 13, 13, 14 ; RED
	dn  9, 13, 13, 13 ; BLUE
	dn  9,  8,  8,  8 ; OFFICER
	dn  7, 14, 10,  8 ; GRUNTF
	dn  9,  8,  8,  8 ; MYSTICALMAN
	dn 12, 15, 15, 15 ; AGATHA (hp ground)
	dn  9,  8,  8,  8 ; ENGINEER
	dn 13, 13, 13, 13 ; GIOVANNI
	dn 13, 13, 13, 13 ; KURT
	dn 14, 15, 15, 15 ; LORELEI (hp electric)
	dn  9,  8,  8,  8 ; SOLDIER
	dn  9,  8,  8,  8 ; SPORTSMAN
	dn  9,  8,  8,  8 ; TEACHER_M
	dn 13, 13, 13, 13 ; GREEN
	dn 14, 15, 15, 15 ; YELLOW (hp electric)
	dn 12, 12, 15, 15 ; PETREL (hp ground)
	dn 13, 13, 15, 15 ; PROTON (hp bug)
	assert_table_length NUM_TRAINER_CLASSES
